...
char *duplicate(char *str){
    char *new_str = malloc(strlen(str) + 1); // +1 for the '\0' char  
    if (new_str == NULL){ // testing to see if malloc failed
        return NULL;
    }
    strcpy(new_str, str); // copying the old string into the new string;
    return new_str;
}