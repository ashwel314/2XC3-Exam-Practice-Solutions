...
void remove_filename(char *url){
    char *p = url;
    while (*p){ // finding the end of the sting
        p++;
    }

    while (*p != '/' && p > url){ // going backwards until the first '/' char
        p--; 
    }

    if (*p == '/'){ // replacing the last '/' char with a new '\0' char to signal the end of the new string 
        *p = '\0';
    }
}